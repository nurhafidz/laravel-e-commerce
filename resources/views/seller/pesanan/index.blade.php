                                    ...
                                    @forelse($orderdetail as $key=>$item)
                                        <tr class="text-gray-700 dark:text-gray-400">
                                            <td class="px-4 py-3 text-sm">
                                                {{ Str::limit($item->order->user->first_name,15) }}</td>
                                            ...
                                            <td>
                                                @if($s == "PAID" || $s == "SETTLED")
                                                  @php $a = Auth::user()->store->name; $storename = str_replace(' ','-',$a) @endphp 
                                                    <a href="{{url('/seller/'.$storename.'/pesanan/'.$item->id)}}" class="bg-green-500 hover:bg-green-700 text-white  px-4 rounded-full">
                                                      Detail
                                                    </a> 
                                                    @endif
                                            </td>
