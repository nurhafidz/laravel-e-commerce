                    ...
                    <div class="col-span-3 lg:col-span-1">
                        <div class="flex flex-wrap -mx-2 overflow-hidden bg-white p-2 shadow rounded-lg dark:divide-gray-700 dark:bg-gray-800">
                            ...
                            <div class="my-2 px-2 md:w-1/2 overflow-hidden">
                                <p class="text-xl dark:text-gray-200">Detail customer</p>
                            </div>
                        ...
                        <div class="flex flex-col bg-white p-4 shadow rounded-lg dark:divide-gray-700 dark:bg-gray-800">
                            <div class="flex flex-col items-center">
                                <div class="inline-flex shadow-lg border border-gray-200 rounded-full overflow-hidden h-40 w-40">
                                    <img src="{{ url('storage/app/public/image/'.$customer->foto) }}" alt="" class="h-full w-full">
                                </div>
                                <h2 class="mt-4 font-bold text-xl dark:text-gray-200">{{$customer->first_name}} {{$customer->last_name}}</h2>
                                <h6 class="mt-2 text-sm font-medium dark:text-gray-200">{{$customer->role->name}}</h6>
                            </div>
                            <div class="flex mt-3 flex-wrap -mx-1 overflow-hidden sm:-mx-2">
                                ...
                                <div class="my-1 px-1 w-full overflow-hidden sm:my-2 sm:px-2 sm:w-1/2 md:w-1/2 lg:w-1/2 xl:w-1/2">
                                    <p class="text-gray-500 dark:text-gray-200">Email : {{$customer->email}}</p>
                                </div>

                                <div class="my-1 px-1 w-full overflow-hidden sm:my-2 sm:px-2 sm:w-1/2 md:w-1/2 lg:w-1/2 xl:w-1/2">
                                    <p class="text-gray-500 dark:text-gray-200">Status : @if ($customer->status == 1)
                                ...
                                <div class="my-1 px-1 w-full overflow-hidden sm:my-2 sm:px-2 sm:w-1/2 md:w-1/2 lg:w-1/2 xl:w-1/2">
                                    <p class="text-gray-500 dark:text-gray-200">Tempat, tanggal lahir : {{$customer->tempat_lahir}}, {{$customer->tanggal_lahir}}</p>
                                </div>
                                <div class="my-1 px-1 w-full overflow-hidden sm:my-2 sm:px-2 sm:w-1/2 md:w-1/2 lg:w-1/2 xl:w-1/2">
                                    <p class="text-gray-500 dark:text-gray-200">No telepon : {{$customer->telepon}}</p>
                                </div>
                                <div class="my-1 px-1 w-full overflow-hidden ">
                                    <p class="text-gray-500 dark:text-gray-200">Alamat :</p>
                                    <p class="text-gray-500 dark:text-gray-200">{{$customer->alamat_lengkap}}, {{$customer->district->name}}, {{$customer->district->city->name}},  {{$customer->district->city->province->name}} {{$customer->kode_pos}}</p>
                                </div>
	...
	<!-- Modal -->
        <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 transform translate-y-1/2" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0  transform translate-y-1/2" @click.away="closeModal" @keydown.escape="closeModal" class="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl" role="dialog" id="modal">
            ...
            <div class="mt-4 mb-6">
                ...
                <!-- Modal description -->
                <p class="text-sm text-gray-700 dark:text-gray-400 normal-case">Apakah anda yakin akan @if ($customer->status == 0)
                    mengaktifkan @else menonaktifkan @endif {{$customer->role->name}} ini ?
                </p>
            </div>
            <footer class="flex flex-col items-center justify-end px-6 py-3 -mx-6 -mb-4 space-y-4 sm:space-y-0 sm:space-x-6 sm:flex-row bg-gray-50 dark:bg-gray-800">
                ...
                <form action="{{url('/admin/customer/'.$customer->id.'/edit-status')}}" method="post">
                    @csrf
                    @method('PUT')
